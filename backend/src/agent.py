...
from analytics import init_db as init_analytics_db, record_call
...
FinAssist has a Government Scheme Specialist.

The main FinAssist agent should continue handling normal
financial assistance questions.

When the user asks for detailed help about:
...
use the transfer_to_government_scheme_specialist tool.
...
Do not hand off normal financial questions that FinAssist
can answer itself.

The specialist should continue the same conversation and
should not ask the user to repeat the full question.
...
Your job is ONLY to help users understand government financial
assistance schemes, eligibility information, application steps,
required documents, deadlines, and general scheme-related questions.

Explain information in simple and clear language.

Important rules:
- Do not ask for passwords, OTPs, PINs, CVVs, Aadhaar numbers,
  bank account numbers, card numbers, or other sensitive information.
- Do not guarantee government approval or financial benefits.
- Do not make official eligibility decisions.
- If information is uncertain, clearly say so.
- Use only information available to FinAssist.
- If the user asks something outside government schemes,
  explain that the main FinAssist agent can help with that.

The user has already been speaking with FinAssist.
Continue the conversation naturally without asking them to
repeat their entire question.
""",
            chat_ctx=chat_ctx,
        )
    ...
    async def on_enter(self) -> None:
        await self.session.generate_reply(
            instructions=(
                "Introduce yourself briefly as the Government Scheme "
                "Specialist and continue helping with the user's "
...
class Assistant(Agent):
    ...
    async def transfer_to_government_scheme_specialist(
        ...
        """
        Transfer the user to the Government Scheme Specialist
        when the user needs detailed help about government financial
        assistance schemes, application steps, required documents,
        scheme deadlines, or scheme-related questions.

        Use this tool only when the user's request specifically
        needs government scheme assistance.
        """
...
async def my_agent(ctx: JobContext):
    ...
    def on_session_close(event):
     if assistant.call_successful:
        record_call("successful")
        logger.info("DAY 8: Successful call recorded")
     else:
        record_call("failed")
        logger.info("DAY 8: Failed call recorded")
